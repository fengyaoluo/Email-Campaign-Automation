...
try:
    ...
    storage_dir_IT = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/storagegroup/Digital Marketing/Emails/One Month Free Email Campaign/Email Lists/{} - OMF - IT.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    DMWA_dir_IT = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/DMWA/Storage/Emails/One Month Free Email Campaign/{} - OMF - IT.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    df_final.to_excel(storage_dir_IT, index=False)

    #Copy to the DWMA folder
    shutil.copy2(storage_dir_IT, DMWA_dir_IT)

    N_IT_final=len(df_final.index)
    ...
    df2city_final.to_excel(storage_dir_IT_2city, index=False, engine='xlsxwriter')
