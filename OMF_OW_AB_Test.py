...
try:
    ...
    fourcity = ["Austin, TX", "Houston, TX", "Little Rock, AR", "Chicago, IL"]
    len(fourcity)
    ...
    for i, row in df_final_control.iterrows():
        df_final_control.at[i,"url2"]=df_final.at[i,"url"]+book_online_link+today

    for i, row in df_final_control.iterrows():
        df_final_control.at[i,"url3"]=df_final.at[i,"url"]+entity_name_link+today

    for i, row in df_final_control.iterrows():
        df_final_control.at[i,"url4"]=df_final.at[i,"url"]+book_storage_button+today

    for i, row in df_final_control.iterrows():
        df_final_control.at[i,"url5"]=df_final.at[i,"url"]+hero+today
    ...
    #Add URL tracking for OW - testB
    book_online_link="?utm_campaign=storage&utm_source=omfOW&utm_medium=book_online_link_B&utm_content="

    entity_name_link="?utm_campaign=storage&utm_source=omfOW&utm_medium=entity_name_link_B&utm_content="

    book_storage_button="?utm_campaign=storage&utm_source=omfOW&utm_medium=book_storage_button_B&utm_content="

    hero="?utm_campaign=storage&utm_source=omfOW&utm_medium=hero_B&utm_content="
    ...
    for i, row in df_final_testB.iterrows():
        df_final_testB.at[i,"url2"]=df_final.at[i,"url"]+book_online_link+today

    for i, row in df_final_testB.iterrows():
        df_final_testB.at[i,"url3"]=df_final.at[i,"url"]+entity_name_link+today

    for i, row in df_final_testB.iterrows():
        df_final_testB.at[i,"url4"]=df_final.at[i,"url"]+book_storage_button+today

    for i, row in df_final_testB.iterrows():
        df_final_testB.at[i,"url5"]=df_final.at[i,"url"]+hero+today
    ...
    storage_dir_OW_control = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/storagegroup/Digital Marketing/Emails/One Month Free Email Campaign/Email Lists/{} - OMF - OW - Control.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    DMWA_dir_OW_control = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/DMWA/Storage/Emails/One Month Free Email Campaign/{} - OMF - OW - Control.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    df_final_control.to_excel(storage_dir_OW_control, index=False)

    #Copy to the DWMA folder
    shutil.copy2(storage_dir_OW_control, DMWA_dir_OW_control)

    #testB
    storage_dir_OW_testB = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/storagegroup/Digital Marketing/Emails/One Month Free Email Campaign/Email Lists/{} - OMF - OW - TestB.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    DMWA_dir_OW_testB = "\\\\adfs01.uhi.amerco/INTERDEPARTMENT/DMWA/Storage/Emails/One Month Free Email Campaign/{} - OMF - OW - TestB.xlsx".format(
        datetime.datetime.strftime(datetime.date.today(), "%m.%d.%y")
    )

    df_final_testB.to_excel(storage_dir_OW_testB, index=False)

    #Copy to the DWMA folder
    shutil.copy2(storage_dir_OW_testB, DMWA_dir_OW_testB)

    N_OW_final=len(df_final_control.index)+len(df_final_testB.index)
